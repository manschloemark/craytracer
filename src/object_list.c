...
object_list object_list_new(memory_region *region, int max_objects) {
	...
	ol.objects = empty_pointer_array;
	ol.count = 0;
	ol.max = max_objects;
	if (ol.objects == NULL) {
		ol.max = -1;
...
object_list *add_object_list(memory_region *region, int max_objects) {
	...
	if(ol.objects == NULL) return NULL;
	object **to_free = ol.objects;
	ol.objects = (object **)memory_region_add(region, ol.objects, max_objects * sizeof(object *));
	free(to_free);
