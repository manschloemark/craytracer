...
double Floordouble(double n) {
	return (n > 0.0) ? (double)(int)n : (double)((int) (n - 1.0));
}
int FloorInt(double n) {
...
void permute(int *perm, int pointcount, thread_context *thread) {
	...
	for(i = pointcount - 1; i > 0; --i) {
		int swap_index = (int)(random_double_between(0.0, (double)i+1.0, &thread->rand_state));
		int temp = perm[i];
...
perlin perlin_new(int pointcount, thread_context *thread) {
	...
	for (i = 0; i < p.pointcount; ++i) {
		p.random_vectors[i] = vec3_unit(vec3_bounded_random(-1.0, 1.0, &thread->rand_state));
	}
...
double shirley_perlin_noise(void *src, point3 *pt) {
	...
	double u = pt->x - Floordouble(pt->x);
	double v = pt->y - Floordouble(pt->y);
	double w = pt->z - Floordouble(pt->z);
	...
	double result = perlin_interpolate(c, u, v, w);
	return result;
}


double perlin_noise(void *src, point3 *pt) {
	perlin *perl = (perlin *)src;
	double u = pt->x - Floordouble(pt->x);
	double v = pt->y - Floordouble(pt->y);
	double w = pt->z - Floordouble(pt->z);
	...
	double uu = u * u * u * (u * (6.0 * u - 15.0) + 10.0);
	double vv = v * v * v * (v * (6.0 * v - 15.0) + 10.0);
	double ww = w * w * w * (w * (6.0 * w - 15.0) + 10.0);

	double dot000 = vec3_dot(v000, vec3_new(u, v, w));
	double dot100 = vec3_dot(v100, vec3_new(u - 1.0, v, w));
	double dot010 = vec3_dot(v010, vec3_new(u, v - 1.0, w));
	double dot110 = vec3_dot(v110, vec3_new(u - 1.0, v - 1.0, w));
	double dot001 = vec3_dot(v001, vec3_new(u, v, w - 1.0));
	double dot101 = vec3_dot(v101, vec3_new(u - 1.0, v, w - 1.0));
	double dot011 = vec3_dot(v011, vec3_new(u, v - 1.0, w - 1.0));
	double dot111 = vec3_dot(v111, vec3_new(u - 1.0, v - 1.0, w - 1.0));

	double result = LERP(LERP(LERP(dot000, dot100, uu), LERP(dot010, dot110, uu), vv), LERP(LERP(dot001, dot101, uu), LERP(dot011, dot111, uu), vv), ww);
	return result;
}
...
double perlin_interpolate_with_derivative(vec3 c[2][2][2], double u, double v, double w, vec3 *d) {
	...
	double d_uu = u * 6.0 * (1.0 - u);
	double d_vv = v * 6.0 * (1.0 - v);
	double d_ww = w * 6.0 * (1.0 - w);
	...
	for (int i = 0; i < 2; ++i) {
		...
		for (int j = 0; j < 2; ++j) {
			...
			for (int k = 0; k < 2; ++k) {
				double fk = (double)k;
				vec3 weight = vec3_new(u - fi, v - fj, w - fk);
				double dot = vec3_dot(c[i][j][k], weight);
				double l_u = (fi * uu + (1.0 - fi) * (1.0 - uu));
				double l_v = (fj * vv + (1.0 - fj) * (1.0 - vv));
				double l_w = (fk * ww + (1.0 - fk) * (1.0 - ww));
...
double shirley_perlin_noise_with_derivative(void *src, point3 *pt, vec3 *d) {
	...
	double result = perlin_interpolate_with_derivative(c, u, v, w, d);
	return result;
}


// NOTE : this might be wrong. Specifically vectors with names vXXX
double perlin_noise_with_derivative(void *src, point3 *pt, vec3 *d) {
	perlin *perl = (perlin *)src;
	double u = pt->x - Floordouble(pt->x);
	double v = pt->y - Floordouble(pt->y);
	double w = pt->z - Floordouble(pt->z);
	...
	double d_uu = u * u * u * (u * u * (u - 2.0) + 1.0);
	double d_vv = v * v * v * (v * v * (v - 2.0) + 1.0);
	double d_ww = w * w * w * (w * w * (w - 2.0) + 1.0);
...
simplex simplex_new(int pointcount, thread_context *thread) {
	...
	for (int i = 0; i < s.pointcount; ++i) {
		s.gradients[i] = vec3_unit(vec3_bounded_random(-1.0, 1.0, &thread->rand_state));
	}
...
double simplex_noise(void *src, vec3 *pt) {
	simplex *simp = (simplex *)src;
	double s = (pt->x + pt->y + pt->z) * simp->skew;
	int i = FloorInt(pt->x + s);
	int j = FloorInt(pt->y + s);
	int k = FloorInt(pt->z + s);

	double t = ((double)(i + j + k)) * simp->unskew;
	double u = ((double)i) - t;
	double v = ((double)j) - t;
	double w = ((double)k) - t;
	double x0 = pt->x - u;
	...
	double x1 = x0 - ((double)i1) + simp->unskew;
	double y1 = y0 - ((double)j1) + simp->unskew;
	double z1 = z0 - ((double)k1) + simp->unskew;
	double x2 = x0 - ((double)i2) + 2.0*simp->unskew;
	double y2 = y0 - ((double)j2) + 2.0*simp->unskew;
	double z2 = z0 - ((double)k2) + 2.0*simp->unskew;
	double x3 = x0 - 1.0 + 3.0*simp->unskew;
	double y3 = y0 - 1.0 + 3.0*simp->unskew;
	double z3 = z0 - 1.0 + 3.0*simp->unskew;
	...
	double t0 = cont - x0*x0 - y0*y0 - z0*z0;
	if (t0 < 0.0)
		n0 = 0.0, dot0 = 0.0;
	else {
	...
	double t1 = cont - x1*x1 - y1*y1 - z1*z1;
	if (t1 < 0.0)
		n1 = 0.0, dot1 = 0.0;
	else
	{
	...
	double t2 = cont - x2*x2 - y2*y2 - z2*z2;
	if (t2 < 0.0)
		n2 = 0.0, dot2 = 0.0;
	else
	{
	...
	double t3 = cont - x3*x3 - y3*y3 - z3*z3;
	if (t3 < 0.0)
		n3 = 0.0, dot3 = 0.0;
	else
	{
	...
	double result =  scale * (n0 + n1 + n2 + n3);
	return result;
}

double simplex_noise_with_derivative(void *src, vec3 *pt, vec3 *d) {
	simplex *simp = (simplex *)src;
